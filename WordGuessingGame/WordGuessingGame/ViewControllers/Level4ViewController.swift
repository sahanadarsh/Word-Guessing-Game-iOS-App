...
class Level4ViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    override func viewDidLoad() {
        ...
        tempImageView.frame = self.level4TblView.frame
        self.level4TblView.backgroundView = tempImageView;
        let settings = FirestoreSettings()
        Firestore.firestore().settings = settings
        db = Firestore.firestore()
        initializeText()
        level4TblView.delegate = self
    ...
    @IBAction func logoutAction(_ sender: Any) {
        do {
            ...
            self.performSegue(withIdentifier: "level4ToLoginSegue", sender: self)
        } catch{
            print (error)
        }
    }
    ...
    func getWords(){
        ...
        getLevel4Words(url)
            .done { (words) in
                ...
                self.level4TblView.reloadData()
                let randomIndex = Int(arc4random_uniform(UInt32(14)))
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = Bundle.main.loadNibNamed("Level4TableViewCell", owner: self, options: nil)?.first as! Level4TableViewCell
    ...
    func eachButtonAlert(word: String) -> (){
        if(isTwoWordsSame(s1: self.secretWord, s2: word) && self.limitCount > 0){
            ...
            let nextLevel = UIAlertAction(title: "NEXT LEVEL", style: .default) { (alertAction) in
                self.initializeText()
                self.performSegue(withIdentifier: "Level4to5Segue", sender: self)
            }
